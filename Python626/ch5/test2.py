...
#함수정의
# def myFunc():
# messagebox.showinfo("다이얼 로그 제목 창입니다. !", "다이얼 로그 내용창입니다.")

def myFunc() :
    if chk.get() == 0:
        messagebox.showinfo("다이얼 로그 제목창입니다. !", "체크버튼 해제. chk 값 : %d" %(chk.get()))
    else :        
        messagebox.showinfo("다이얼 로그 제목창입니다. !", "체크버튼 선택. chk 값 : %d" %(chk.get()))

def myFunc2():
    if var.get() == 1 :
        resultLabel.configure(text="라디오버튼1 var 값 : %d" %(chk.get()))
    elif var.get() == 2:
        resultLabel.configure(text="라디오버튼2 var 값 : %d" %(chk.get()))
    elif var.get() == 3:
        resultLabel.configure(text="라디오버튼3 var 값 : %d" %(chk.get()))    
...
button1.pack(sid=TOP, fill=X, padx=10, pady=10, ipadx=10, ipady=10)
button3.pack(sid=TOP, fill=X, padx=10, pady=10, ipadx=10, ipady=10)
