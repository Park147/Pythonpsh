...
h = photo.height()
w = photo.width()
print('이미지 크기 (h x w): ', h, 'x', w)

photoR=[ [0 for _ in range(h)] for _ in range(w)] # _ = index i or index k
photoG=[ [0 for _ in range(h)] for _ in range(w)]
photoB=[ [0 for _ in range(h)] for _ in range(w)]

for i in range(w) :
    for k in range(h) :
        r, g, b = photo.get(i,k) # 비구조화 할당, 함수 리턴하면 1개, 파이썬 특성 중 여러 개를 반환함 type 튜플 (js도 그러하다.)
        photoR[i][k] = r
        photoG[i][k] = g
        photoB[i][k] = b
